...
angular.module('manage').controller('manageConnectionGroupController', ['$scope', '$injector', 
    ...
    var ConnectionGroupUser = $injector.get('ConnectionGroupUser');
    ...
    $scope.connectionGroupUser = null;
    ...
    $scope.selectedUserFlag = [];
    $scope.removedUserFlag = [];
    /**
    ...
    if (identifier) {
        ...
        connectionGroupService.getConnectionGroupUsers($scope.selectedDataSource, identifier)
        .success(function connectionGroupUsersReceived(connectionGroupUser) {
            $scope.connectionGroupUser = connectionGroupUser;
            $scope.connectionGroupUser.userMap={};
            ConnectionGroupUser.prepare($scope.connectionGroupUser.addUsers,$scope.connectionGroupUser.userMap);            
        });
    }
    ...
    // If we are creating a new connection group, populate skeleton connection group data
        ...
        $scope.connectionGroupUser = new ConnectionGroupUser();
    }
    ...
    $scope.groupListContext = {
        getPermissionFlags : function getPermissionFlags() {
            return $scope.connectionGroupUser;
        },

       connectionGroupUserPermissionChanged : function connectionGroupUserPermissionChanged(identifier) {
            var granted = $scope.connectionGroupUser.addUsers[identifier];
            if (granted){
            	$scope.removedUserFlag.push(identifier);
            }
            else {
            	$scope.selectedUserFlag.push(identifier);
            }
        }
    ...
    $scope.saveConnectionGroup = function saveConnectionGroup() {
        // Save the connection
    	var connectionGroupUser = new ConnectionGroupUser();
    	connectionGroupUser.addUsers=$scope.selectedUserFlag;
    	connectionGroupUser.removeUsers=$scope.removedUserFlag;
        connectionGroupService.saveConnectionGroup($scope.selectedDataSource, $scope.connectionGroup, connectionGroupUser)
